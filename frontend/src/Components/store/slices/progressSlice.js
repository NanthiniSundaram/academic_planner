...
const progressSlice = createSlice({
  ...
  reducers: {
    addStudySession: (state, action) => {
      ...
      state.metrics.averageProductivity =
        state.studySessions.reduce((sum, session) => sum + session.productivity, 0) / state.studySessions.length;
    },
    updateStudySession: (state, action) => {
      const index = state.studySessions.findIndex(session => session.id === action.payload.id);
      if (index !== -1) {
        state.studySessions[index] = action.payload;
      }
    },
    deleteStudySession: (state, action) => {
